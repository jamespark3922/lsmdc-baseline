...
class MultiModalGenerator(CaptionModel):
    def __init__(self, opt):
        ...
        self.context_encoding_size = self.rnn_size // 4
        self.sent_rnn = self.rnn_cell(2 * self.input_encoding_size + self.context_encoding_size, self.rnn_size,
        ...
        self.context_encoder = nn.Linear(self.rnn_size, self.context_encoding_size)

        # entire encoder
        self.encoder = nn.Linear(3 * self.rnn_size, self.input_encoding_size)
    ...
    def get_visual_embeddings(self):
        visual_embeddings = []
        if self.use_video:
            visual_embeddings.append(self.frame_embed)
        if self.use_img:
            visual_embeddings.append(self.image_embed)
        if self.use_box:
            visual_embeddings.append(self.box_embed)
    ...
    def _forward(self, fc_feats, img_feats, box_feats, seq):
        ...
        for n in range(sent_num):
            ...
            for i in range(seq.size(2)-1):
                ...
                encoded = self.encoder(torch.cat((video, image, box), dim=2))
            ...
            if self.context:
                context = self.context_encoder(self.get_hidden_state(state))
    ...
    def _sample(self, fc_feats, img_feats, box_feats, opt={}):
        sample_max = opt.get('sample_max', 1)
        beam_size = opt.get('beam_size', 1)
        temperature = opt.get('temperature', 1.0)
        if sample_max and beam_size > 1:
            return self._sample_beam(fc_feats, img_feats, box_feats, opt=opt)
        batch_size = fc_feats.size(0)
        sent_num = fc_feats.size(1)
        video = torch.zeros(batch_size, 1, self.rnn_size).cuda()
        image = torch.zeros(batch_size, 1, self.rnn_size).cuda()
        ...
        for n in range(sent_num):
            ...
            for t in range(self.seq_length+1):
                ...
                if sample_max:
                    ...
                    # deal with <unk> which has index vocab_size - 1
    ...
    # Not implemented yet
    def _sample_beam(self, fc_feats, img_feats, box_feats, opt={}):
        ...
        for k in range(batch_size):
            ...
            for n in range(sent_num):  # sent_num
                ...
                output, state = self.sent_rnn(xt, state)
                logprobs = F.log_softmax(self.logit(self.dropout(output.squeeze(1))), dim=1)
                self.done_beams[k] = self.beam_search(state, logprobs, fkn,ikn,bkn,context, opt=opt)  # actual beam search
    ...
    def get_logprobs_state(self, it, fkn, ikn, bkn, context, state):
    ...
    def sample_sequential(self, fc_feats, img_feats, box_feats, context=None, opt={}):
        temperature = opt.get('temperature', 1.0)
        top_num = opt.get('top_num', 1.0)
        ...
        for t in range(self.seq_length + 1):
            ...
            if t == self.seq_length: # skip if we achieve maximum length
                break
            # scale logprobs by temperature
            logprobs[:,self.vocab_size-1].fill_(float('-inf'))
